...
player = {
  ...
  xs = 20,
  ys = 0,
  x = 20, --mudança posicionamento inicial
  y = 25,
  speed = 5,
  dir = "down"
}
...
function love.draw()
    local ftx, fty = (-player.x*32)+(love.graphics.getWidth()/2) + 20, (-player.y*32)+(love.graphics.getHeight()/2) + 20
    love.graphics.push()
    ...
    love.graphics.draw(player.charImage, player.charQuads[player.frame],
                      (love.graphics.getWidth()/2), (love.graphics.getHeight()/2), 0, 1, 1, 20, 20)
 end
